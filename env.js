...
const env = (() => {
  ...
  const GITHUB_APP_ID = parseInt(procesimport dotenv from 'dotenv';
  dotenv.config();
  ...
  const env = (() => {
    ...
    const USERS_TABLE_NAME = process.env.USERS_TABLE_NAME || 'onboard-users-database';
    const USERS_JSON_FILE = process.env.USERS_JSON_FILE || 'users.json';
  ...
  export default env.env.GITHUB_APP_ID || "0", 10)
  const GITHUB_APP_PRIVATE_KEY = process.env.GITHUB_APP_PRIVATE_KEY;
  if (!GITHUB_APP_ID || !GITHUB_APP_PRIVATE_KEY) {
    console.log('GITHUB_APP_ID and GITHUB_APP_PRIVATE_KEY must be defined');
    throw new Error('GITHUB_APP_ID and GITHUB_APP_PRIVATE_KEY must be defined');
